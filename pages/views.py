...
def NewSubmisson (request, group_id=None):
    groupObj = None
    if request.method == 'POST':
        group_form = NewSubmissonForm(request.POST)
        ...
        if request.method == 'GET':
            if group_id:
                ...
                group_form = NewSubmissonForm(instance=groupObj)
            else:
                group_form = NewSubmissonForm()

    return render(request, 'pages/SubmissonForm.html', {'GroupForm_form': group_form,
                                                              'groupobj': groupObj, })
